...
The `BioCroField` package includes several features related to processing raw
data from biomass harvests, with the ultimate goal of producing values that are
useful in the context of crop growth modeling. In particular, the package
...
- Store the commands used to digitize and process the data (such as the ones
  in the [A Full Example] section of this document) as an R script in the same
  directory.

- Save the final biomass data table as a plain-text file in the same directory
  using `write.csv`.
...
directory in the future, including yourself! Even though some details are not
immediately apparent from the script, the documentation for the functions from
`BioCroField` will describe how the calculations were made.
...
section), the general steps required to make these measurements (in the
[Making Measurements] section), and how `BioCroField` can be used to process the
resulting measurements (in the [Using BioCroField] section). The final section
([A Full Example]) shows a realistic example of how the functions from
`BioCroField` can be used to process a full biomass data set from a field
season.
...
also plays a key role in simulating photosynthesis at the canopy level. If the
model is able to reproduce these mass and leaf area measurements, we can have
more faith that it will be able to make accurate yield predictions in other
situations.
...
one time. Using these sheets in the field will help you stay organized, and make
sure your data is organized in a way that will facilitate later analysis. The
record sheet can be accessed from R by using the `access_sheet` function from
`BioCroField` as follows:

```{r access_sheet, eval = FALSE}
access_sheet('harvest')
```

The sheet is also available via the `BioCroField`
[GitHub website](https://github.com/biocro/BioCroField/blob/main/inst/data_record_sheets/harvest.pdf).
...
elements. More information is available via the built-in help system by typing
...
convert its information into a more useful form. The output from `process` is
a `harvest_point` object that has additional named elements that represent the
newly calculated values. More information is available via the built-in help
...
`BioCroField`, which accepts multiple `harvest_point` objects, extracts the most
important information from each one, and stores the information in one row of a
`data.frame`. With the data in this format, it can also be easily saved in a
file using the `write.csv` function from base R. More information is available
via the built-in help system by typing `?biomass_table` in the R terminal.
...
`add_seed_biomass` function from `BioCroField`. More information is available
via the built-in help system by typing `?add_seed_biomass` in the R terminal.
